...
const DB_BASE_NAME = "CodeWhisperDB";
...
class ConversationDB implements DB {
  ...
  async init(): Promise<void> {
    ...
    if (navigator.locks) {
      this.initPromise = navigator.locks.request(
        "codewhisper-db-init",
        async (lock) => {
  ...
  private mergeConversations(
    ...
    const activeConvs = local.filter(
      (conv) =>
        conv.messages && conv.messages.length > 0 && conv.isActive !== false
    );
    ...
    remote.forEach((conv) => {
      ...
      if (existingConv) {
        if (
          existingConv.isActive === false ||
          (existingConv._version &&
            conv._version &&
            existingConv._version > conv._version)
        ) {
          return;
        }
      }
  ...
  async saveConversations(conversations: Conversation[]): Promise<void> {
    ...
    if (navigator.locks) {
      return navigator.locks.request("codewhisper-db-write", async (lock) => {
  ...
  private async _saveConversationsWithLock(
    ...
    try {
      console.debug("Starting save operation:", {
        ...
        hasActiveConversations: conversations.some(
          (c) => c.messages?.length > 0 && c.isActive !== false
        ),
      });
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        tx.oncomplete = () => {
          if (!saveCompleted) {
            reject(
              new Error(
                "Transaction completed but save operation did not complete"
              )
            );
            return;
          }
  ...
  async getConversations(): Promise<Conversation[]> {
    ...
    if (navigator.locks) {
      return navigator.locks.request("codewhisper-db-read", async (lock) => {
  ...
  private async _getConversationsWithLock(): Promise<Conversation[]> {
    ...
    return new Promise<Conversation[]>((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const conversations = Array.isArray(request.result)
          ? request.result
          : [];

        if (conversations.length > 0) {
          const validConversations = conversations.filter(
            (conv) => conv && this.validateConversations([conv])
          );
